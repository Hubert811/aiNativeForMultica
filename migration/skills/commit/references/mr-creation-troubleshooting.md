...
RESPONSE=$(curl -s -w "\n%{http_code}" --request POST \
  ...
  "https://<git-host>/api/v4/projects/pdcl%2F{PROJECT_NAME}/merge_requests" \
...
# 方法 1: 查询特定分支的最新 MR
curl -s --header "PRIVATE-TOKEN: ${GITLAB_PAT}" \
  "https://<git-host>/api/v4/projects/pdcl%2F{PROJECT_NAME}/merge_requests?state=opened&source_branch=feat%2Fepic-15-story-15-19-database-infrastructure&order_by=created_at&sort=desc" \
  | python3 -c "
import sys, json
data = json.load(sys.stdin)
if data:
...
# URL: https://<git-host>/pdcl/{PROJECT_NAME}/-/merge_requests/27
# 飞书任务ID: 6934756567
```
...
# 验证飞书任务 ID 是否正确包含
curl -s --header "PRIVATE-TOKEN: ${GITLAB_PAT}" \
  "https://<git-host>/api/v4/projects/pdcl%2F{PROJECT_NAME}/merge_requests/27" \
  | python3 -c "
import sys, json
data = json.load(sys.stdin)
...
PROJECT_ID="pdcl%2F{PROJECT_NAME}"
...
echo "https://<git-host>/pdcl/{PROJECT_NAME}/-/merge_requests/27"
...
4. **验证结果**（HTTP 201）：
   ...
   # 查询 MR 信息
   curl -s --header "PRIVATE-TOKEN: ${GITLAB_PAT}" \
     "https://<git-host>/api/v4/projects/pdcl%2F{PROJECT_NAME}/merge_requests?state=opened&source_branch=feat%2Fepic-15-story-15-19-database-infrastructure" \
     | python3 -c "
   import sys, json
   data = json.load(sys.stdin)
   mr = data[0]
