...
| 优先级 | 识别方式 | 条件 | 说明 |
|--------|---------|------|------|
| **P0** | Annotation 显式声明 | `cmdb.minieye.tech/resource-type` | 新方案，推荐 |
| **P1** | 旧 Annotation 降级 | `dev-pod/resource-type` | ENHANCEMENT-001 调研发现 |
...
CREATE TABLE pod_resource_status (
    ...
    pod_namespace VARCHAR(64) NOT NULL DEFAULT 'dev-pod',
    pod_resource_type VARCHAR(32) NOT NULL DEFAULT 'DEV_POD',
    ...
    CONSTRAINT chk_pod_resource_type CHECK (pod_resource_type IN ('DEV_POD', 'ARGO_WORKFLOW', 'RAY_JOB', 'TRAIN_JOB', 'UNKNOWN')),
