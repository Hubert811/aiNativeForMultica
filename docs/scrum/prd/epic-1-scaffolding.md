...
{BUSINESS_DESCRIPTION}服务是一个全新的 Go-Zero 微服务项目，需要从零搭建完整的工程化基础设施。参考 dev_pod 项目的成功经验，本服务需要建立标准的项目脚手架，包括代码生成、配置管理、构建系统等基础能力。
...
| 需求 ID | 需求描述 | 指标 |
|---------|---------|------|
| NFR-1-01 | 兼容 dev_pod 项目构建规范 | 100% 兼容 |
...
| 风险 | 影响 | 概率 | 缓解措施 |
|------|------|------|---------|
| goctl 版本兼容性问题 | 高 | 低 | 使用与 dev_pod 相同版本 |
| 配置文件结构设计不合理 | 中 | 中 | 参考 dev_pod 项目配置 |

### 5.2 资源依赖

| 依赖项 | 类型 | 状态 |
|--------|------|------|
| dev_pod 项目 Makefile | 参考 | 已完成 |
| dev_pod 项目 .gitlab-ci.yml | 参考 | 已完成 |

### 5.3 缓解措施

- 提前验证 goctl 版本兼容性
- 配置文件设计与 dev_pod 保持一致
- Makefile 目标与 dev_pod 保持一致
...
- [dev_pod 项目](https://git.minieye.com/pdcl/dev_pod)
- [CLAUDE.md](../CLAUDE.md)
