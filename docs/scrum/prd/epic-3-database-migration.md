...
- DevPod 表结构设计（dev_pod, dev_pod_usage_record）
- 迁移脚本（001, 002, ...）
- 迁移执行脚本（migrate.sh）
- 种子数据脚本
...
-- DevPod 用量记录表
CREATE TABLE dev_pod_usage_record (
    id SERIAL PRIMARY KEY,
    dev_pod_id INTEGER REFERENCES dev_pod(id),
