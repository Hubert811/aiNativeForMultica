...
```yaml
Kubeconfig:
  K8sApiServerUrl: "https://192.168.25.20:6443"   # API Server 地址
  K8sBearerToken: "eyJhbGci..."                  # Bearer Token
  K8sCaFilePath: "./etc/config/ca.crt"           # CA 证书路径
  DevPodNamespace: "dev-pod"                      # 监听的命名空间
```
...
kubectl create serviceaccount {PROJECT_NAME} -n dev-pod

# 验证 Service Account 已创建
kubectl get sa {PROJECT_NAME} -n dev-pod
```
...
kubectl create token {PROJECT_NAME} -n dev-pod --duration=87600h > etc/bearer-token.txt
...
#   "sub": "system:serviceaccount:dev-pod:{PROJECT_NAME}",
#   ...
# }
```
...
curl -k --cacert etc/ca.crt \
  ...
  https://192.168.25.20:6443/api/v1/namespaces/dev-pod/pods \
...
2. 重新生成 Token：`kubectl create token {PROJECT_NAME} -n dev-pod --duration=87600h > etc/bearer-token.txt`

### 问题 3: 权限不足

**错误**:
```
pods is forbidden: User "system:serviceaccount:dev-pod:{PROJECT_NAME}" cannot list resource "pods"
```
