    ...
    if os.path.exists("/var/run/secrets/kubernetes.io/serviceaccount/token"):
        ...
        if not kubeconfig_path:
            raise EnvironmentError(
                ...
                "请使用以下命令之一：\n"
                "  export KUBECONFIG=/home/mini/.kube/test/config\n"
                "  KUBECONFIG=/home/mini/.kube/test/config pytest tests/sit/\n\n"
                "📍 测试环境 kubeconfig 路径：\n"
                "  ✅ 测试环境: /home/mini/.kube/test/config (推荐)\n"
                "  ❌ 生产环境: /home/mini/.kube/prod_gpu/config (严禁使用!)\n\n"
                "💡 提示：在 ~/.bashrc 或 ~/.zshrc 中添加别名：\n"
                "  alias test-kubeconfig='export KUBECONFIG=/home/mini/.kube/test/config'"
            )
            ...
            if "test-config" in kubeconfig_path:
                ...
                if os.path.exists(fixed_path):
                    raise EnvironmentError(
                        ...
                        f"   正确路径: {fixed_path}\n\n"
                        f"💡 修复命令：\n"
                        f"  export KUBECONFIG=/home/mini/.kube/test/config"
                    )
        ...
        def create_pod(self, pod_name, namespace, labels=None, annotations=None,
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    # - pdcl-worker-type=gpu:NoSchedule (52个节点)
                    # - pdcl-worker-type/ide=gpu:NoSchedule (9个节点)
                    # - pdcl-worker-type/drs=gpu:NoSchedule (3个节点)
                    tolerations = None
                    if gpu_count > 0:
                        # 使用 Exists operator 匹配所有 pdcl-worker-type=* 的 Taint
                        tolerations = [
                            client.V1Toleration(
                                key="pdcl-worker-type",
                                operator="Exists",
                                effect="NoSchedule"
                            )
                        ]
...
def create_devpod(k8s_client):
    """创建 DevPod 的 fixture"""
    def _create(pod_name, gpu_count=1, namespace="dev-pod", node_selector=None, labels=None, annotations=None):
        default_labels = {
            "app.kubernetes.io/name": "dev-pod",
            "dev-pod.minieye.cc/managed": "true",
        }
        ...
        # ✅ 修复: 使用 v3.2 标准的 cmdb.minieye.tech/ annotation 前缀
        # 参考文档: docs/design/cmdb_design_v3.0.md
        default_annotations = {
            "cmdb.minieye.tech/resource-name": pod_name,
            "cmdb.minieye.tech/user-id": "test-user-sit",
            "cmdb.minieye.tech/user-name": "Test User SIT",
            "cmdb.minieye.tech/user-email": "test-sit@example.com",
            "cmdb.minieye.tech/team-id": "test-team-sit",
            "cmdb.minieye.tech/team-name": "Test Team SIT",
            "cmdb.minieye.tech/project-id": "test-project-sit",
            "cmdb.minieye.tech/project-name": "Test Project SIT",
        }
...
def delete_pod(k8s_client):
    """删除 Pod 的 fixture"""
    def _delete(pod_name, namespace="dev-pod"):
...
def clean_k8s_pods(k8s_client, prefix, max_retries=2):
    ...
    """
    # 测试相关的 namespace
    namespaces = ["dev-pod", "argo", "dcs", "default", "train-job"]
...
def abort_pod(k8s_client):
    ...
    """
    def _abort(pod_name, namespace="dev-pod"):
        try:
