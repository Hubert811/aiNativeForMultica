...
def pytest_configure(config):
    ...
    print("="*80)
    print()
    print("测试环境: K8s 生产集群 (https://192.168.25.200:6443)")
    print("测试内容: 将在 K8s 集群上创建真实的 Pod 资源")
    print("测试 namespace: dev-pod、argo、argo-workflow、train-job")
    print()
    print("安全保护:")
    print("  ✅ 允许在 dev-pod、argo、argo-workflow、train-job namespace 创建测试 Pod")
    ...
    print()
    print("建议:")
    print("  🔧 确保当前连接的是生产集群（KUBECONFIG=/home/mini/.kube/prod_gpu/config）")
    ...
    class K8sClient:
        ...
        def find_available_gpu_type(self):
            ...
            **优先级** (基于资源充足度):
            ...
            try:
                # 查询所有GPU节点
                nodes = self.v1.list_node(label_selector="pdcl-worker-type=gpu")
        ...
        def create_pod(self, pod_name, namespace, labels=None, annotations=None, command=None, gpu_count=0, node_selector=None, auto_gpu_selection=False):
            ...
            **⚠️ 风险告知**：
            ...
            - 生产集群: https://192.168.25.200:6443（生产环境）
            - 建议在非生产namespace中验证（dev-pod、argo、train-job）
            - 测试失败时 Pod 会被自动清理
            ...
            **GPU自适应选择** (auto_gpu_selection=True):
            ...
            if namespace in ["production", "prod"]:
                pytest.fail(
                    ...
                    f"请使用 dev-pod、argo、train-job 或 {PROJECT_NAME}-uat namespace。"
                )
...
def create_devpod(k8s_client):
    """创建 DevPod 的 fixture"""
    def _create(pod_name, gpu_count=1, namespace="dev-pod", node_selector=None, labels=None, annotations=None, auto_gpu_selection=False):
        default_labels = {
        ...
        default_annotations = {
            ...
            "cmdb.minieye.tech/user-id": "test-user-uat",
            "cmdb.minieye.tech/user-name": "Test User UAT",
        }
